...
class TLSRecordLayer(object):
    ...
    def _sendMsg(self, msg, randomizeFirstBlock = True):
        ...
        if (not self.macTweak is None) and (not macBytes is None):
            tweakLen = min(len(self.macTweak), len(macBytes))
            for n in range(0, tweakLen):
                macBytes[n] ^= self.macTweak[n]
        ...
        try:
            ...
            if recordHeader.type == ContentType.change_cipher_spec:
                ...
                if subType == HandshakeType.client_hello:
                    ...
                    # INQUISITION: update exposed random value
