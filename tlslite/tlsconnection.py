...
class TLSConnection(TLSRecordLayer):
    ...
    def _sendFinished(self, masterSecret, cipherSuite=None, nextProto=None):
        ...
        # tweak the MAC if we need to
        if not self.macTweak is None:
            tweakLen = min(len(verifyData), len(self.macTweak))
            for i in range(0, tweakLen):
                verifyData[i] ^= self.macTweak[i]
